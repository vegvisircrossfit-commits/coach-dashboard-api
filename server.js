...
// ── Roster cache endpoint ─────────────────────────────────────────────────────
// Serves the pre-built roster from roster.py cache file
const fs = require("fs");
const ROSTER_CACHE = process.env.ROSTER_CACHE_FILE || "/tmp/roster_cache.json";

app.get("/roster/:classId", (req, res) => {
  try {
    if (!fs.existsSync(ROSTER_CACHE)) {
      return res.status(404).json({ error: "Roster not built yet — check back 15 min before class" });
    }
    const cache = JSON.parse(fs.readFileSync(ROSTER_CACHE, "utf8"));
    const cls = cache.classes?.[req.params.classId];
    if (!cls) {
      return res.status(404).json({ error: "Class not found in cache", available: Object.keys(cache.classes || {}) });
    }
...
app.get("/roster", (req, res) => {
  try {
    if (!fs.existsSync(ROSTER_CACHE)) {
      return res.status(404).json({ error: "Roster not built yet" });
    }
    const cache = JSON.parse(fs.readFileSync(ROSTER_CACHE, "utf8"));
    // Return summary of all classes without full athlete data
    const summary = Object.entries(cache.classes || {}).map(([id, cls]) => ({
      class_id: id,
      class_name: cls.class_name,
      start_time: cls.start_time,
      athlete_count: cls.athletes?.length || 0,
    }));
